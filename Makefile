...
# get version form git tags and repo's state
...
prepare::
	...
	if [[ "${OS}" != "Darwin" ]]; then \
		sed -i 's,github.com/pulumi/pulumi-proxmox,${REPOSITORY},g' provider/go.mod; \
...
install_resource_plugin::
	...
	pulumi plugin install resource ${PACK} ${VERSION} -f $(WORKING_DIR)/bin/${PROVIDER}-${VERSION}.tgz
